# Germline filter: filter V gene candidates to produce final germline database
# Uses abstract type + multiple dispatch for composable filters
...
function should_discard end  # interface
...
function should_discard(f::CrossMappingFilter, ref::FilterCandidate,
                       cand::FilterCandidate, ::Bool)
    s_end = min(ref.cdr3_start, length(ref.sequence))
    t_end = min(cand.cdr3_start, length(cand.sequence))
    s_no = SubString(ref.sequence, 1, s_end)
    t_no = SubString(cand.sequence, 1, t_end)

    dist = if length(s_no) != length(t_no)
        short_len = min(length(s_no), length(t_no))
        min(edit_distance(s_no[1:short_len], t_no[1:short_len]; maxdiff = 1),
            edit_distance(s_no[max(1,end-short_len+1):end], t_no[max(1,end-short_len+1):end]; maxdiff = 1))
    else
        edit_distance(s_no, t_no; maxdiff = 1)
    end
...
function mark_rows!(df::DataFrame, condition::AbstractVector{Bool}, reason::String)
    ...
    @info "Marked $(sum(condition)) candidates as '$reason'"
end

"""
    germline_filter!(candidates, criteria; whitelist) -> (filtered, annotated)

Apply germline filtering. Returns (filtered_df, annotated_df).
"""
function germline_filter!(candidates::DataFrame, criteria::GermlineFilterCriteria;
    ...
    # Per-row filters
    mark_rows!(candidates, candidates.CDR3s_exact .< criteria.unique_cdr3s, "too_low_CDR3s_exact")
    mark_rows!(candidates, candidates.Js_exact .< criteria.unique_js, "too_low_Js_exact")

    if hasproperty(candidates, :CDR3_shared_ratio)
        ratios = [parse(Float64, string(x)) for x in candidates.CDR3_shared_ratio]
        mark_rows!(candidates, ratios .> criteria.cdr3_shared_ratio, "too_high_CDR3_shared_ratio")
    end

    if !criteria.allow_stop
        mark_rows!(candidates,
    ...
    fc = [FilterCandidate(
        ...
        !ismissing(candidates.database_diff[i]) && candidates.database_diff[i] == 0,
...
function germline_filter_to_fasta(filtered::DataFrame, path::AbstractString)
    records = [FastaRecord(row.name, row.consensus) for row in eachrow(filtered)]
