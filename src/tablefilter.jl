...
Base.:(+)(a::FilterStats, b::FilterStats) = FilterStats(
    a.total + b.total, a.has_vj_assignment + b.has_vj_assignment,
    a.has_no_stop + b.has_no_stop, a.good_v_evalue + b.good_v_evalue,
    a.good_v_coverage + b.good_v_coverage, a.good_j_coverage + b.good_j_coverage,
    a.has_cdr3 + b.has_cdr3)

"""
    filter_table(df::DataFrame, pf::PreprocessingFilter) -> (DataFrame, FilterStats)
...
"""
    filter_table(input, output, pf; stats_path) — file-based variant
"""
function filter_table(input_path::AbstractString, output_path::AbstractString,
                     pf::PreprocessingFilter;
                     stats_path::String = "")
    df = read_assignments(input_path)
    filtered, stats = filter_table(df, pf)

    endswith(output_path, ".gz") ? write_table_gzipped(output_path, filtered) :
